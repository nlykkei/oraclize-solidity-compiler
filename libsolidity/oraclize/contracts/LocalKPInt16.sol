/** 
 *  @file    LocalKPInt16.sol
 *  @author  Nicolas Lykke Iversen (nlykkei)
 *  @date    1/1/2018  
 *  @version 1.0 
 *  
 *  @brief Oraclize, Local "kp" benchmark (int16). 
 *
 *  @section DESCRIPTION
 *  
 *  Contract for benchmarking local "kp" (int16).
 */

pragma solidity ^0.4.11;

contract LocalKPInt16 {
            
    //event LogEvent(string msg, uint[] res);

    int16 constant INF = -1;

    function LocalKPInt16() public payable {}    
  
    /// Local k-path
    function kp(int16[] w, uint k, uint W) public {
        uint[] memory _kp = kpath(w, k, W);
        //LogEvent("local", _kp);
    }
    
    /// Path P of length k, w(P) <= W 
    function kpath(int16[] w, uint k, uint W) private constant returns(uint[]) {
    ...
    /// Path reconstruction    
    function getPath(int16[] w, uint k, uint W, int[][][] sp, uint n) private constant returns(uint[]) {
        uint i; uint j; uint e; uint a;
